...
CWordSearchDlg::CWordSearchDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CWordSearchDlg::IDD, pParent)
{
...
void CWordSearchDlg::WordSearch(CString strInput, CStringList* pstrOutput)
{
	strInput.MakeLower(); // 입력 소문자 통일
	...
	while(Inputpos != NULL )
	{
		CString strTemp = string_list.GetAt(Inputpos);

		strTemp.MakeLower();
		if( _tcsstr(strTemp,strInput) )
		{
			pstrOutput->AddTail(string_list.GetAt(Inputpos));
