...
CSliderControlDlg::CSliderControlDlg(CWnd* pParent /*=NULL*/)
		: CDialogEx(CSliderControlDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CSliderControlDlg, CDialogEx)
		ON_WM_SYSCOMMAND()
		ON_WM_PAINT()
		ON_WM_QUERYDRAGICON()
		ON_WM_HSCROLL()
...
BOOL CSliderControlDlg::OnInitDialog()
{
		...
		m_editCtrl.SetWindowText(_T("0"));

		return TRUE;  // 포커스를 컨트롤에 설정하지 않으면 TRUE를 반환합니다.
}

void CSliderControlDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CSliderControlDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
		...
		if(pScrollBar != NULL && pScrollBar->m_hWnd == m_sliderCtrl.m_hWnd){
				CString str;
				str.Format(_T("%d"), m_sliderCtrl.GetPos());
...
BOOL CSliderControlDlg::PreTranslateMessage(MSG* pMsg)
{
